...
public class Check : MonoBehaviour
{
    ...
    //shouldn't matter checking pong first, since it is strictly = not >3. This method is made so you don't need multiple loops to check for similar item
    public int CheckPongKongForAllPlayers()
    {
        ...
        return 10; //equiv of no pongs
    }

    public void RemoveFromHand()
    {
        int player = convert.PlayerConvNumber(TurnMJ.turn);
        ...
        {
            tile.transform.position = convert.PlayerConvRemoveBox(TurnMJ.turn).transform.position;
            hands.PlayerHands[player].revealedchips.Add(tile);
            hands.PlayerHands[player].playerchips.Remove(tile);
        }
        hands.PlayerHands[4].playerchips[hands.PlayerHands[4].playerchips.Count - 1].transform.position = convert.PlayerConvRemoveBox(TurnMJ.turn).transform.position; //move taken tile out of middle
